...
# ===== secondary_notifier (Discord sidecar) =====


@pytest.mark.asyncio
async def test_secondary_notifier_fires_on_primary_success(db):
    """primary (LINE) 送信成功時に secondary (Discord) にも並行送信される。"""
    await _create_confirmed_event(db)
    primary = FakeNotifier()
    secondary = FakeNotifier()
    disp = NotificationDispatcher(
        lottery_repo=LotteryEventRepo(db),
        product_repo=ProductRepo(db),
        notification_repo=NotificationRepo(db),
        notifier=primary,
        secondary_notifier=secondary,
        max_per_run=10,
        max_per_day=150,
    )
    result = await disp.dispatch(now=datetime(2026, 4, 21, 12, 5))
    ...
    # 同じ summary が両方に流れる
    assert primary.sent[0] == secondary.sent[0]


@pytest.mark.asyncio
async def test_secondary_not_fired_when_primary_suppressed(db):
    """primary が dedupe で suppressed の場合、secondary も送らない (LINE dedupe を継承)。"""
    await _create_confirmed_event(db)
    primary = FakeNotifier()
    secondary = FakeNotifier()
    disp = NotificationDispatcher(
    ...
    # 1回目: 両方送られる
    await disp.dispatch(now=datetime(2026, 4, 21, 12, 5))
    # 2回目: primary dedupe で suppress → secondary も来ない
    await disp.dispatch(now=datetime(2026, 4, 21, 12, 10))
    assert len(primary.sent) == 1
    assert len(secondary.sent) == 1


@pytest.mark.asyncio
async def test_secondary_failure_does_not_break_primary(db):
    """secondary が例外を投げても primary の結果には影響しない (fire-and-forget)。"""
    await _create_confirmed_event(db)
    ...
    disp = NotificationDispatcher(
        ...
        notification_repo=NotificationRepo(db),
        notifier=primary,
        secondary_notifier=FailingNotifier(),
        max_per_run=10,
        max_per_day=150,
    )
    result = await disp.dispatch(now=datetime(2026, 4, 21, 12, 5))
...
@pytest.mark.asyncio
async def test_secondary_not_fired_when_primary_is_dry_run(db):
    """DryRunNotifier が primary の場合、secondary は実送信しない (would-send ログを汚さない)。"""
    from pokebot.notify.line import DryRunNotifier
    await _create_confirmed_event(db)
