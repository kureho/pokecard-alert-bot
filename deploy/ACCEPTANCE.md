...
## stale event cleanup の運用手順 (archive-stale-events)

以下3カテゴリの active event を一括で archived にする cleanup job。
lottery_upsert 側でも新規 insert 時に apply_end_at 過去は自動 archive されるが、過去データや disable 済 adapter 由来の orphan が残るため、この job で定期的に掃除する。

1. **non_tokyo_metro**: `src/pokebot/lib/region.py` の allowlist 外の cardlabo / pokemoncenter
2. **apply_ended**: `apply_end_at` が now から 1h 以上過去 (受付終了済み)
3. **disabled_adapter**: retailer が `amazon / pokecawatch / yodobashi / biccamera / amiami / unknown` または NULL、もしくは store_name が `@...` (Twitter username) の orphan

### 推奨手順 (GHA 経由で secrets を漏らさない)

1. **dry-run で件数確認**
   - GitHub → Actions → pokebot → Run workflow
   - job: `archive-stale-events`
   - dry_run: `true` (どちらでもよい。この job は LINE を使わない)
   - cleanup_execute: **false** (必ず false のまま)
   - Run → ログで「対象 N 件」「reason ごとの件数」「retailer / store 内訳」を確認
2. **内容を確認し、問題なければ実行**
   - 同じ workflow を再実行
...
```sql
-- cleanup 実行日に archived になった event だけ active に戻す
...
reason カテゴリで絞り込みたい場合は `retailer_name` や `apply_end_at` で追加条件を足す。

### 対象判定ロジック

- `status='active'` のみ対象 (archived / pending_review は触らない)
- 優先順位: `apply_ended` > `disabled_adapter` > `non_tokyo_metro`
